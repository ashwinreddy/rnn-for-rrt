...
def train(sess, model, eval_model, train_set, valid_set, test_set):
    ...
    for _ in range(hps.num_steps):
        ...
        if step % 20 == 0 and step > 0:
            ...
            cost_summ.value.add(
                tag='Train_Cost', simple_value=float(train_cost))
            ...
            for summary in [cost_summ, reconstr_summ, kl_summ, lr_summ, kl_weight_summ, time_summ]:
        ...
        if step % hps.save_every == 0 and step > 0:
            ...
            for summary in [valid_cost_summ, valid_reconstr_summ, valid_kl_summ, valid_time_summ]:
                summary_write.add_summary(summary, train_step)
            summary_writer.flush()
            ...
            if valid_cost < best_valid_cost:
                ...
                eval_cost_summ.value.add(
                    tag='Eval_Cost', simple_value=float(eval_cost))
                ...
                output_values = (eval_cost, eval_r_cost,
                                 eval_kl_cost, time_taken_eval)
                output_log = output_format % output_values

                tf.logging.info(output_log)

                for summary in [eval_cost_summ, eval_reconstr_summ, eval_kl_summ, eval_time_summ]:
                    summary_writer.add_summary(summary, train_step)
                summary_writer.flush()
